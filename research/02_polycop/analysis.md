...
flowchart TD
    ...
    B --> C[⚠️ 无需注册账号]
    C --> D[需要准备两个信息]
    D --> D1[① 自己的 Polymarket 私钥]
    D --> D2[② 自己的 Polymarket Proxy 地址]
    D1 --> E[如何获取私钥]
...
flowchart TD
    ...
    note1[⚠️ PolyCop 自身不提供聪明钱发现功能，这是其最大短板]
```
...
flowchart TD
    ...
    I --> I2[或使用 MoonPay 出金]
    note1[提现前需确认所有持仓已平仓或到期结算]
    note2[PolyCop 收取的 0.05% 费用已在盈利时自动扣除]
```
