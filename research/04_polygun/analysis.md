...
flowchart TD
    ...
    G1 --> H[注册完成，等待充值]
    %% 注: ⚠️ 托管模式：私钥由 Polygun 服务端加密保管
    %% 注: 用户无需 MetaMask，无需任何 Web3 知识
```
...
flowchart TD
    ...
    E --> G[Bot 推送充值到账通知]
    G --> H[余额更新，可开始交易]
    %% 注: 跨链桥费用由 Polygun 扣除或计入手续费
    %% 注: 到账时间: Polygon 直接约 1min，跨链约 3-10min
```
...
flowchart TD
    ...
    K --> M[Bot 推送提现成功通知]
    L --> M
    %% 注: ⚠️ 持仓中的资产无法直接提现，结算后自动入账
    %% 注: 1% 手续费按交易量收取，提现本身可能另有手续费
```
